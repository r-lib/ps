...
ps implements an API to query and manipulate system processes. Most of
...
[`ps_pids()`](https://ps.r-lib.org/reference/ps_pids.md) returns all
process ids on the system. This can be useful to iterate over all
processes.
...
[`ps()`](https://ps.r-lib.org/reference/ps.md) returns a data frame,
with data about each process. It contains a handle to each process, in
the `ps_handle` column, you can use these to perform more queries on the
processes.
...
The essential [`ps_pid()`](https://ps.r-lib.org/reference/ps_pid.md),
[`ps_create_time()`](https://ps.r-lib.org/reference/ps_create_time.md),
[`ps_is_running()`](https://ps.r-lib.org/reference/ps_is_running.md)
functions and the [`format()`](https://rdrr.io/r/base/format.html) and
[`print()`](https://rdrr.io/r/base/print.html) methods work for all
...
The [`ps_ppid()`](https://ps.r-lib.org/reference/ps_ppid.md),
[`ps_parent()`](https://ps.r-lib.org/reference/ps_ppid.md),
...
[`ps_children()`](https://ps.r-lib.org/reference/ps_children.md) and the
signal sending functions work properly for zombie processes. Other
functions fail with `"zombie_process"` error.
...
Using [`ps()`](https://ps.r-lib.org/reference/ps.md) and dplyr:
...
[`ps_wait()`](https://ps.r-lib.org/reference/ps_wait.md), from ps 1.8.0,
implements a new way, efficient for waiting on a list of processes, so
...
Again, this is much simpler with
[`ps_wait()`](https://ps.r-lib.org/reference/ps_wait.md), added in ps
1.8.0.
...
From ps 1.8.0, [`ps_kill()`](https://ps.r-lib.org/reference/ps_kill.md)
will first send `SIGTERM` signals on Unix, and `SIGKILL` after a grace
period, if needed.
